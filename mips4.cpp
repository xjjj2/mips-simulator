...
unsigned char br_pre[17][257] = { 0 };
unsigned char _label[17] = { 0 };
class data_haz{};
bool iftaken;
class con_haz{
};
...
void beq(int n, int t) {
	if (t == 3) {
		tmp[3].a[1] = (tmp[3].a[1] == tmp[3].a[2] ? 1 : 0);
		int x = tmp[3].pc % 16;
		int t = br_pre[x][_label[x]];
		if (tmp[3].a[1] == 1 && br_pre[x][_label[x]] < 3) ++br_pre[x][_label[x]];
		if (tmp[3].a[1] == 0 && br_pre[x][_label[x]] > 0) --br_pre[x][_label[x]];
		_label[tmp[3].pc % 16] = (_label[tmp[3].pc % 16] << 1) + tmp[3].a[1];
		if (tmp[3].a[1] == 1 && t < 2) throw con_haz();
		if (tmp[3].a[1] == 0 && t >= 2) throw con_haz();
	}
}
...
/*	while (npc < cnt) {
	...
	while (npc < cnt) {
		...
		try {
			...
			clog << ernum << '/' << total << endl;
			clog << double(ernum) / double(total) << endl;
			if (x.h) return x.val;
			else return 0;
		}
		catch (data_haz) {
